 ...
 * \return A handle to the process, if successful; if not, an error code. The rights of the handle change according to
 *         the relation this process has to the target process:
 ...
 */
int hydrogen_process_find(int id, uint32_t flags) __asm__("__hydrogen_process_find");
 ...
 *
 * \param[in] flags The flags that should be set on the returned handle.
 * \return A handle to the newly created process, if successful; if not, a negative error code.
 */
int hydrogen_process_create(uint32_t flags) __asm__("__hydrogen_process_create");
 ...
 */
int hydrogen_process_getpid(int process) __asm__("__hydrogen_process_getpid");

/**
 * Get the ID of a process's parent.
 *
 * \param[in] process The process whose parent's ID to get. Can be #HYDROGEN_THIS_PROCESS.
 * \return The ID of the process's parent.
 */
int hydrogen_process_getppid(int process) __asm__("__hydrogen_process_getppid");

/**
 * Get the ID of a process's group.
 *
 * \param[in] process The process whose group's ID to get. Can be #HYDROGEN_THIS_PROCESS.
 * \return The ID of the process's group.
 */
int hydrogen_process_getpgid(int process) __asm__("__hydrogen_process_getpgid");

/**
 * Get the ID of a process's session.
 *
 * \param[in] process The process whose session's ID to get. Can be #HYDROGEN_THIS_PROCESS.
 * \return The ID of the process's session.
 */
int hydrogen_process_getsid(int process) __asm__("__hydrogen_process_getsid");
 ...
 * \return The ID of the new session, if successful; if not, a negative error code.
 */
int hydrogen_process_setsid(int process) __asm__("__hydrogen_process_setsid");
 ...
 * \return The size of the process's supplementary group list (in `integer`), if successful; if not, an error code
 *         (in `error`). Note that the returned value may be larger than `count`; if so, the first `count` group IDs
 *         have been placed in `buffer`.
 */
